# H5 Forge Reference - Existing Project First Scan
...
首次接入时必须补一层 H5 专项扫描，不能只看通用目录结构：

```bash
python3 scripts/project_snapshot.py /path/to/h5/project --json
```

扫描结果只作为规则卡 evidence，不直接覆盖用户已确认规则。具体检测范围和 confidence 写入规则见 [h5_stack_detection.md](h5_stack_detection.md)。
...
- `references/h5_stack_detection.md`
